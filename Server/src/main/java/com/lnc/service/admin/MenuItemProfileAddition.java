...
public class MenuItemProfileAddition {
    ...
    public String addMenuItemProfile(String jsonData) {
        try {
            ...
            if(menuItemProfileQueries.addMenuItemProfile(menuItemProfile, itemId)) {
                return "Added menu item profile.";
            } else {
                return "Failed to add menu item profile.";
            }
        } catch (JsonProcessingException | NullPointerException ex) {
            logger.severe("Error in adding menu item profile: " + ex.getMessage());
            return "Error in adding menu item profile.";
        }
    }
}
