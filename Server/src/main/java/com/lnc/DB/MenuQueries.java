...
public class MenuQueries {
    private static final Logger logger = Logger.getLogger(MenuQueries.class.getName());
    ...
    public boolean checkMenuItemPresent(String item) {
        ...
        try (PreparedStatement checkMenuItemPresentStmt = connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = checkMenuItemPresentStmt.executeQuery()) {
                if (rs.next()) {
    ...
    public List<Map<String, Object>> viewMenuItems() {
        ...
        try (PreparedStatement viewMenuItemsStmt = connection.prepareStatement(query)) {
            try (ResultSet rs = viewMenuItemsStmt.executeQuery()) {
                while (rs.next()) {
    ...
    public int getItemID(String menuItem) {
        ...
        try (PreparedStatement getItemIDStmt = connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = getItemIDStmt.executeQuery()) {
                if (rs.next()) {
    ...
    public List<Integer> getAllMenuIds() {
        ...
        try (PreparedStatement getAllMenuIdsStmt = connection.prepareStatement(query)) {
            try (ResultSet rs = getAllMenuIdsStmt.executeQuery()) {
                while (rs.next()) {
